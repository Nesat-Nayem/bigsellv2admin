...
import { useGetDisclaimerQuery, useUpdateDisclaimerMutation } from '@/store/disclaimerApi'
...
const DisclaimerPage = () => {
  ...
  const { data: privacyPolicy, isLoading, isError } = useGetDisclaimerQuery()
  const [updatePrivacyPolicy, { isLoading: isUpdating }] = useUpdateDisclaimerMutation()
  ...
  const handleSave = async () => {
    try {
      ...
      showMessage('Disclaimer updated successfully', 'success')
    } catch (error) {
      console.error('Error updating Disclaimer:', error)
      showMessage('Failed to update Disclaimer', 'error')
    }
  }

  if (isLoading) {
    return <div className="text-center py-5">Loading Disclaimer...</div>
  }

  if (isError) {
    return <div className="text-center py-5 text-danger">Error loading Disclaimer</div>
  }
